            ...
            try {
                ...
                switch (choice) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        System.out.println("\nLes teves pel·lícules disponibles:");
                        LinkedHashSet<Movie> availableMovies = manager.filterMovies("");

                        if (availableMovies.isEmpty()) {
                            System.out.println("No hi ha pel·lícules disponibles.");
                        } else {
                            int index = 1;
                            ArrayList<Movie> movieList = new ArrayList<>(availableMovies);
                            for (Movie movie : movieList) {
                                System.out.println(index + ". " + movie.getTitle());
                                index++;
                            }
                            System.out.print("\nSelecciona la teva pel·lícula preferida (introdueix el número): ");
                            int selectedNumber = sc.nextInt();
                            sc.nextLine();

                            if (selectedNumber > 0 && selectedNumber <= movieList.size()) {
                                Movie selectedMovie = movieList.get(selectedNumber - 1);
                                manager.getCurrentUser().setFavouriteMovie(selectedMovie);
                                System.out.println("S'ha establert " + selectedMovie.getTitle() + " com la teva pel·lícula preferida.");
                            } else {
                                System.out.println("Número invàlid. Torna-ho a intentar.");
                            }
                        }
                        enterToContinue(sc);
                        break;
